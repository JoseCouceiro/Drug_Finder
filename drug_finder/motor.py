...
class Motor:
    ...
    def __display1(self):
        __choice = input("\nYou may search for a medicine or an active compound (select 1 or 2):\n"
                        "  1. Search a medicine\n"
                        "  2. Search an active compound\n"
                        "  3. Search by therapeutic indication\n"
                        "  4. Exit\n")
        
        __nchoices = 4
        __output = self.__check_input(__choice, __nchoices)
        self.__process_choice1(__output)

    def __process_choice1(self, selection):   
        if selection == '1':
            __search_term = self.__searcher.find_medicines()
        ...
        self.__productive_search = self.__printer.med_list(self.__searcher.response_dict)
