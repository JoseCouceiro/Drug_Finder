    ...
    def get_compound_props(self, compound_name):
        ...
        for __comp in __compounds:
            ...
            for __tup in zip(__prop_list,list(self.__prop_dict.values())):
                print(f"   {__tup[0]}: {__tup[1]}")

        return __compounds

    def get_png(self, compound_name, compound_list):
        for __comp in compound_list:
            pcp.download('PNG',resources.path("drug_finder.outputs",f"{compound_name}_{__comp.cid}.png"),f'{__comp.inchikey}','inchikey',image_size = 'large',overwrite=True)
        
        __insert = ''   
        if len(compound_list) > 1 :
            __insert = 'of each compound '        
        print(f'\nAn image of the molecular structure {__insert}has been downloaded to the outputs folder\n')
